...
#============ cookie optional ============#
    ...
    def cookies(self):
        all_cookie = self.headers['Cookie']
        slime = re.split('; ', all_cookie)
        return dict( [ (item[0], item[1]) for item in [ cookie.split('=', 1) for cookie in slime ] ] )
...
class API(URL, DATA, COOKIE, SEND):
    ...
    def do_OPTIONS(self):
        headers = {
            ...
            'Connection': 'close',
            'Access-Control-Allow-Origin': 'https://yiyan.baidu.com',
            'Access-Control-Allow-Headers': 'Content-Type,x-bd-plugin-sessionidhash,x_bd_logid'
        }
        self.send_code(200)
